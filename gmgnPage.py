...
def gmgnPage():
    ...
    # market_value = int(total_supply) * float(price)
    # print(market_value)
    ...
    db = SessionLocal()
    addresses = crud.get_coin_base_address(db=db, quote_symbol='SOL', slice=1)
    ...
    for coin in js_res['data']:
        market_value = int(coin['total_supply']) * float(coin['price']['price'])
        if market_value > 1000000000: # 1b
            asyncio.run(sendMsg('1b', coin))
        if market_value > 100000000: # 100m
            asyncio.run(sendMsg('100m', coin))
        if market_value > 10000000: # 10m
            asyncio.run(sendMsg('10m', coin))
        # if market_value > 6300: # test
        #     asyncio.run(sendMsg('test', coin))
